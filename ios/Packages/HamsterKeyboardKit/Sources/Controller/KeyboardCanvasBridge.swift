...
enum KeyboardCanvasInputState: String, Codable {
  case idle
  case launching
  case drawing
  case processing
...
struct KeyboardCanvasResultPayload: Codable {
...
private struct KeyboardCanvasSessionPayload: Codable {
  let requestId: String
  let state: KeyboardCanvasInputState
  let updatedAt: TimeInterval
  let errorMessage: String?
}

/// 键盘侧画布桥接，负责从主 App 拉取导出的 JPG 结果。
final class KeyboardCanvasBridge {
  static let shared = KeyboardCanvasBridge()
  ...
  private enum Constants {
    ...
    static let lastInsertedRequestIdKey = "canvas.input.last_inserted_request_id"
  ...
  func setState(requestId: String, state: KeyboardCanvasInputState, errorMessage: String? = nil) {
    let payload = KeyboardCanvasSessionPayload(
      requestId: requestId,
      state: state,
  ...
  func readLatestUnconsumedResult() -> KeyboardCanvasResultPayload? {
    ensureDirectories()
    guard let activeRequestId = activeRequestId() else { return nil }
    guard let payload = read(KeyboardCanvasResultPayload.self, from: resultFileURL(for: activeRequestId)) else {
      return nil
    }
    guard payload.consumed == false else { return nil }
    guard !payload.imageRelativePath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
      return nil
    }
    return payload
  }

  func resolveImageURL(relativePath: String) -> URL {
    FileManager.shareURL.appendingPathComponent(relativePath)
  }

  func markResultConsumed(requestId: String) {
    guard var payload = read(KeyboardCanvasResultPayload.self, from: resultFileURL(for: requestId)) else { return }
    guard payload.consumed == false else { return }

    let now = Date().timeIntervalSince1970
    payload = KeyboardCanvasResultPayload(
      requestId: payload.requestId,
      imageRelativePath: payload.imageRelativePath,
    ...
    userDefaults.set(KeyboardCanvasInputState.inserted.rawValue, forKey: Constants.stateKey)
  }
...
private extension KeyboardCanvasBridge {
