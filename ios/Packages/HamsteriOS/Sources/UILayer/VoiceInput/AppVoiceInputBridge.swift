...
public enum AppVoiceInputState: String, Codable {
  case idle
  case launching
  case recording
  case processing
...
private struct AppVoiceInputSessionPayload: Codable {
  let requestId: String
  let state: AppVoiceInputState
  let updatedAt: TimeInterval
  let errorMessage: String?
}

private struct AppVoiceInputResultPayload: Codable {
  let requestId: String
  let text: String
...
/// 主 App 侧语音桥接，负责深链路由与结果落盘。
public final class AppVoiceInputBridge {
  public static let shared = AppVoiceInputBridge()
  ...
  public func setState(requestId: String, state: AppVoiceInputState, errorMessage: String? = nil) {
    let payload = AppVoiceInputSessionPayload(
      requestId: requestId,
      state: state,
  ...
  public func writeResult(requestId: String, text: String, localeIdentifier: String?) {
    let now = Date().timeIntervalSince1970
    let payload = AppVoiceInputResultPayload(
      requestId: requestId,
      text: text,
